...
final class CoinRewardService {
    ...
    private func showCoinAnimation(amount: Int, reason: String, on viewController: UIViewController) {
        ...
        let (container, glowView) = makeCoinContainer(in: overlay)
        let coinImageView = makeCoinImageView(in: container)
        attachCoinLabels(amount: amount, reason: reason, to: container, below: coinImageView)
        ...
        animateCoinPresentation(overlay: overlay, container: container, coinImageView: coinImageView, glowView: glowView)
    }

    private func makeCoinContainer(in overlay: UIView) -> (container: UIView, glowView: UIView) {
        ...
        return (container, glowView)
    }

    private func makeCoinImageView(in container: UIView) -> UIImageView {
    ...
    private func attachCoinLabels(amount: Int, reason: String, to container: UIView, below coinImageView: UIImageView) {
    ...
    private func animateCoinPresentation(
        overlay: UIView,
        container: UIView,
