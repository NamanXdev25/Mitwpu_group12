...
extension HealthProfileModel {
    init(from profile: ProfileUserProfile) {
        ...
        gender = Gender(rawValue: profile.gender) ?? .female

        let formatter = DateFormatter()
        formatter.dateFormat = "dd MMM yyyy"
        diagnosisDate = formatter.date(from: profile.diagnosisDate)
        ...
        treatmentPhase = TreatmentPhase(rawValue: phaseName) ?? .recentlyDiagnosed
    }
}
