...
extension HealthStatusViewController {
    ...
    private func registerCells() {
        ...
        for item in ["HealthFieldCell", "HealthDateCell", "HealthDropdownCell"] {
            collectionView.register(
...
extension HealthStatusViewController: UICollectionViewDataSource {
    ...
    func collectionView(
        ...
        if indexPath.section == 0 {
            return userProfileHeaderCell(collectionView, at: indexPath)
        }

        switch indexPath.item {
        case 0: return firstNameCell(collectionView, at: indexPath)
        case 1: return lastNameCell(collectionView, at: indexPath)
        case 2: return diagnosisDateCell(collectionView, at: indexPath)
        case 3: return genderCell(collectionView, at: indexPath)
        case 4: return dobCell(collectionView, at: indexPath)
        default: return treatmentPhaseCell(collectionView, at: indexPath)
        }
    }

    private func userProfileHeaderCell(_ collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "UserProfileHeaderCell", for: indexPath) as? UserProfileHeaderCell else {
            fatalError("Expected UserProfileHeaderCell for reuse identifier 'UserProfileHeaderCell' at \(indexPath)")
        }
        cell.configure(name: "\(profile.firstName) \(profile.lastName)", image: profile.profileImage)
        cell.onImageTap = { [weak self] in
            guard let self = self, self.isEditingProfile else { return }
            self.showImagePicker()
        }
        return cell
    }

    private func firstNameCell(_ collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "HealthFieldCell", for: indexPath) as? HealthFieldCell else {
            fatalError("Expected HealthFieldCell for reuse identifier 'HealthFieldCell' at \(indexPath)")
        }
        cell.configure(title: "First Name", value: profile.firstName, isEditing: isEditingProfile)
        cell.delegate = self
    ...
    private func lastNameCell(_ collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "HealthFieldCell", for: indexPath) as? HealthFieldCell else {
            fatalError("Expected HealthFieldCell for reuse identifier 'HealthFieldCell' at \(indexPath)")
        }
        cell.configure(title: "Last Name", value: profile.lastName, isEditing: isEditingProfile)
        cell.delegate = self
    ...
    private func diagnosisDateCell(_ collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "HealthDateCell", for: indexPath) as? HealthDateCell else {
            fatalError("Expected HealthDateCell for reuse identifier 'HealthDateCell' at \(indexPath)")
        }
        cell.configure(title: "Date of diagnosis", date: profile.diagnosisDate, isEditing: isEditingProfile)
        addSeparator(to: cell)
        cell.onDateChanged = { [weak self] d in self?.profile.diagnosisDate = d }
        return cell
    }

    private func genderCell(_ collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "HealthDropdownCell", for: indexPath) as? HealthDropdownCell else {
            fatalError("Expected HealthDropdownCell for reuse identifier 'HealthDropdownCell' at \(indexPath)")
        }
        cell.configure(title: "Gender", value: profile.gender.rawValue, isEditing: isEditingProfile)
        addSeparator(to: cell)
        cell.onDropdownTap = { [weak self] in
            guard let self = self, self.isEditingProfile else { return }
            self.showOptionsPopup(
                title: "Gender", options: Gender.allCases.map { $0.rawValue }, selected: self.profile.gender.rawValue, sourceCell: cell
            ) { [weak self] chosen in
    ...
    private func dobCell(_ collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "HealthDateCell", for: indexPath) as? HealthDateCell else {
            fatalError("Expected HealthDateCell for reuse identifier 'HealthDateCell' at \(indexPath)")
        }
        cell.configure(title: "Date of Birth", date: profile.dateOfBirth, isEditing: isEditingProfile)
        addSeparator(to: cell)
        cell.onDateChanged = { [weak self] d in self?.profile.dateOfBirth = d }
        return cell
    }

    private func treatmentPhaseCell(_ collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "HealthDropdownCell", for: indexPath) as? HealthDropdownCell else {
            fatalError("Expected HealthDropdownCell for reuse identifier 'HealthDropdownCell' at \(indexPath)")
        }
        cell.configure(title: "Treatment Phase", value: profile.treatmentPhase.rawValue, isEditing: isEditingProfile)
        cell.layer.sublayers?.filter { $0.name == "sep" }.forEach { $0.removeFromSuperlayer() }
        cell.onDropdownTap = { [weak self] in
            guard let self = self, self.isEditingProfile else { return }
            self.showOptionsPopup(
                title: "Treatment Phase", options: TreatmentPhase.allCases.map { $0.rawValue }, selected: self.profile.treatmentPhase.rawValue, sourceCell: cell
            ) { [weak self] chosen in
    ...
    private func addSeparator(to cell: UICollectionViewCell) {
        ...
        line.frame = CGRect(
            x: 16,
            y: cell.bounds.height - 0.5,
            width: cell.bounds.width - 32,
            height: 0.5
        )
...
extension HealthStatusViewController: UICollectionViewDelegateFlowLayout {
    func collectionView(
        ...
        let cardW = fullW - 40
        return CGSize(
            width: indexPath.section == 0 ? fullW : cardW,
...
final class OptionsPopupViewController: UIViewController,
    ...
    init(
        ...
        optionTitle = title; self.options = options; self.selected = selected
