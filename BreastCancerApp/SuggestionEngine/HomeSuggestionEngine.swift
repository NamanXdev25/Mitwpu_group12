...
enum HomeSuggestionEngine {
    ...
    private static func breathingPhaseScore(title: String, ctx: AppContext) -> Int {
        ...
        case "chemotherapy": return chemoPhaseScore(t)
        case "surgery": return surgeryPhaseScore(t)
        case "radiation": return radiationPhaseScore(t)
        case "hormone": return hormonePhaseScore(t)
        case "immunotherapy", "targeted": return immunoPhaseScore(t)
        case "stemcell": return stemcellPhaseScore(t)
        case "earlyDiagnosis": return earlyDiagnosisPhaseScore(t)
        default: return defaultPhaseScore(t, ctx: ctx)
        }
    }

    private static func chemoPhaseScore(_ t: String) -> Int {
        switch t {
    ...
    private static func surgeryPhaseScore(_ t: String) -> Int {
        switch t {
    ...
    private static func radiationPhaseScore(_ t: String) -> Int {
        switch t {
    ...
    private static func hormonePhaseScore(_ t: String) -> Int {
        switch t {
    ...
    private static func immunoPhaseScore(_ t: String) -> Int {
        switch t {
    ...
    private static func stemcellPhaseScore(_ t: String) -> Int {
        switch t {
    ...
    private static func earlyDiagnosisPhaseScore(_ t: String) -> Int {
        switch t {
    ...
    private static func defaultPhaseScore(_ t: String, ctx: AppContext) -> Int {
    ...
    private static func breathingSubtitleOverride(title: String, moodKey: String, ctx: AppContext) -> String? {
        ...
        if let combo = comboSubtitleOverride(t: t, m: m, phase: phase, ctx: ctx) {
            return combo
        }

        if let p = phaseSubtitleOverride(t: t, phase: phase, ctx: ctx) {
            return p
        }

        return moodSubtitleOverride(t: t, m: m)
    }

    private static func comboSubtitleOverride(t: String, m: String, phase: String, ctx: AppContext) -> String? {
        if m == "anxious", ctx.isInActiveTreatment, t == "calmer mind" { return "Slow your exhale to quiet the fear around treatment." }
        if m == "tired", phase == "chemotherapy", t == "gentle recharge" { return "A gentle reset when chemo leaves your body drained." }
        if m == "tired", phase == "chemotherapy", t == "deep rest" { return "A soft session to let your body fully rest after chemo." }
        if m == "tired", phase == "stemcell", t == "deep rest" { return "A gentle session for deep rest during your recovery." }
        if m == "sad", ctx.isInActiveTreatment, t == "healing reflections" { return "A soft rhythm to hold you through the hard days of treatment." }
        if m == "happy", ctx.isPostTreatment, t == "morning appreciation" { return "Breathe in the progress you have made. You earned this." }
        if m == "excited", ctx.isPostTreatment, t == "morning appreciation" { return "Channel your excitement into gratitude for how far you have come." }
        if m == "anxious", ctx.isEarlyDiagnosis, t == "calmer mind" { return "Steady breaths to help you take things one day at a time." }
        return nil
    }

    private static func phaseSubtitleOverride(t: String, phase: String, ctx: AppContext) -> String? {
        if ctx.isPhaseUnknown { return nil }
        ...
        default:
            if ctx.isPostTreatment, t == "healing reflections" { return "A soft session to breathe through what you have been through." }
        }
        return nil
    }

    private static func moodSubtitleOverride(t: String, m: String) -> String? {
        switch m {
        case "anxious":
    ...
    private static func fallbackHobby() -> Suggestion {
        Suggestion(
            imageName: "Reading",
            title: "Read a page of something light",
