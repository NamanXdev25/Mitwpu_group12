...
class JourneyViewController: UIViewController {
    ...
    private func configureTreatmentCell(_ cell: TreatmentCell, for _: IndexPath) {
        ...
        setupTreatmentCellSaveAndStatus(cell)
        setupTreatmentCellFieldsAndReset(cell)
        setupTreatmentCellAddDelete(cell)
    ...
    private func setupTreatmentCellSaveAndStatus(_ cell: TreatmentCell) {
    ...
    private func setupTreatmentCellFieldsAndReset(_ cell: TreatmentCell) {
    ...
    private func setupTreatmentCellAddDelete(_ cell: TreatmentCell) {
...
extension JourneyViewController: UICollectionViewDataSource {
    ...
    func collectionView(
        ...
        case 0: return createDiagnosisCell(collectionView, at: indexPath)
        case 1: return createWaitCell(collectionView, at: indexPath)
        case 2: return createTreatmentCell(collectionView, at: indexPath)
        default: return createPostTreatmentCell(collectionView, at: indexPath)
        }
    }

    private func createDiagnosisCell(_ collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "DiagnosisCell", for: indexPath) as? DiagnosisCell else { return UICollectionViewCell() }
        cell.configure(with: diagnosisModel)
        cell.setLocked(false)
        cell.onDateSelected = { [weak self] date in self?.diagnosisModel.diagnosisDate = date }
        cell.onSaveButtonTapped = { [weak self] in
            guard let self else { return }
    ...
    private func createWaitCell(_ collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "WaitCell", for: indexPath) as? WaitCell else { return UICollectionViewCell() }
        cell.onSaveButtonTapped = { [weak self] days, symptoms in
            ...
            UIView.performWithoutAnimation { self.collectionView.reloadItems(at: [IndexPath(item: 1, section: 0)]) }
        }
        cell.onWaitPeriodExpired = { [weak self] in
            guard let self else { return }
            JourneyState.shared.completeWait()
            DispatchQueue.main.async {
                UIView.performWithoutAnimation { self.collectionView.reloadItems(at: [IndexPath(item: 2, section: 0)]) }
            }
        }
        cell.onEditButtonTapped = { [weak self] in
            guard let self else { return }
            ...
            self.savedPostTreatmentState = SavedPostTreatmentState()
            UIView.performWithoutAnimation {
                self.collectionView.reloadItems(at: [IndexPath(item: 1, section: 0), IndexPath(item: 2, section: 0), IndexPath(item: 3, section: 0)])
            }
        }
        cell.onCellHeightChanged = { [weak self] in
            guard let self else { return }
    ...
    private func createTreatmentCell(_ collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "TreatmentCell", for: indexPath) as? TreatmentCell else { return UICollectionViewCell() }
        configureTreatmentCell(cell, for: indexPath)
        return cell
    }

    private func createPostTreatmentCell(_ collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "PostTreatmentCell", for: indexPath) as? PostTreatmentCell else { return UICollectionViewCell() }
...
extension JourneyViewController {
    private func presentPostTreatmentCalendar() {
        let (dimView, container) = createCalendarPopupViews()
        let datePicker = setupDatePicker(in: container)
        setupCalendarButtons(in: container, datePicker: datePicker, dimView: dimView)
    }

    private func createCalendarPopupViews() -> (UIView, UIView) {
        ...
        return (dimView, container)
    }

    private func setupDatePicker(in container: UIView) -> UIDatePicker {
    ...
    private func setupCalendarButtons(in container: UIView, datePicker: UIDatePicker, dimView: UIView) {
    ...
    private func dismissCalendarPopup(clearDate: Bool) {
        ...
        UIView.animate(withDuration: 0.25, animations: { dimView.alpha = 0 }, completion: { _ in
