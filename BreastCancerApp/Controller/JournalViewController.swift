...
class JournalViewController: UIViewController {
    ...
    func openGuidedJournal() {
        ...
        let loadingAlert = UIAlertController(title: nil, message: "Loading reflection...", preferredStyle: .alert)
        ...
        // Fetch today's question (now async with Apple Suggestions)
        GuidedReflectionDataSource.shared.getTodaysQuestion { [weak self] question in
            DispatchQueue.main.async {
                loadingAlert.dismiss(animated: true) {
                    ...
                    vc.categoryText = self.formattedCategoryTags(for: question)
    ...
    func formattedCategoryTags(for question: GuidedReflectionQuestion) -> String {
        ...
        // Show "APPLE SUGGESTION" tag if it's from Apple
        if question.appleSuggestionID != nil {
            return "INTELLIGENT SUGGESTION • \(categoryText)"
        }
