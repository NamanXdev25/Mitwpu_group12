...
class HealthInsightsViewController: UIViewController, UIAdaptivePresentationControllerDelegate {
    ...
    private func presentDatePicker() {
        ...
        guard let pickerVC = navController.viewControllers.first
                as? InsightDatePickerViewController else {
            assertionFailure("Root VC is not InsightDatePickerViewController")
            return
        }
        if case .week(let anchor) = activeFilter {
    ...
    @objc private func handleCareDataUpdated() {
        if Thread.isMainThread { loadData() }
        else { DispatchQueue.main.async { [weak self] in self?.loadData() } }
    }
...
extension HealthInsightsViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView,
                        numberOfItemsInSection section: Int) -> Int { healthInsights.count }
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        case .hydration:
            let cell = collectionView.dequeueReusableCell(
                withReuseIdentifier: "HydrationCell", for: indexPath
            ) as! HydrationCollectionViewCell
        ...
        // MARK: Exercise — uses MedicationCell with status icons, same as medication screen
        case .exercise:
            let cell = collectionView.dequeueReusableCell(
            ...
            cell.titleLabel.text         = insight.title
            cell.subtitleLabel.text      = insight.subtitle
            cell.adherenceRateLabel.text = insight.mainValue       // "X / 7"
            cell.metricCaptionLabel.text = insight.secondaryValue  // "Days active"
            cell.dosesTakenLabel.text    = insight.medicationTaken  // "X exercises done"
            cell.dosesMissedLabel.text   = insight.medicationMissed // "· X today"
            // 1 = exercised ✓, 0 = did not ○, -1 = future —
...
private extension HealthInsightsViewController {
    ...
    static func makeSymptomsPopupText(for date: Date) -> NSAttributedString {
        ...
        if logs.isEmpty {
            result.append(NSAttributedString(string: "No symptoms logged",
        ...
        for (i, s) in summaries.enumerated() {
            result.append(NSAttributedString(string: "\(s.name) : \(s.severity)",
                attributes: [.foregroundColor: primaryColor,
                             .font: UIFont.systemFont(ofSize: 14, weight: .medium),
                             .paragraphStyle: bodySty]))
